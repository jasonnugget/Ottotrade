...
import { stockProfile } from './stockProfiles.js';
...
const MIN_RETURNS_FOR_VOLATILITY = 20;
...
// The stock's own normal trading range, measured from its actual price history. This is
// what makes an event move interpretable: without it, a -3% day looks alarming for every
// ticker, when for a high-beta name it's a Tuesday and for a low-vol blue chip it isn't.
function volatilityProfile(history) {
  const closes = (history?.bars || []).map((bar) => bar.c).filter((close) => close > 0);
  const returns = [];
  ...
  const medianAbsDailyMovePct = sortedAbs[Math.floor(sortedAbs.length / 2)] * 100;

  let peak = closes[0];
  let maxDrawdown = 0;
  for (const close of closes) {
    if (close > peak) peak = close;
    maxDrawdown = Math.max(maxDrawdown, (peak - close) / peak);
  }

  return {
    dailyVolPct: round2(dailyVolPct),
    annualizedVolPct: round2(dailyVolPct * Math.sqrt(TRADING_DAYS_PER_YEAR)),
    medianAbsDailyMovePct: round2(medianAbsDailyMovePct),
    maxDrawdownPct: round2(maxDrawdown * 100),
    tradingDaysSampled: returns.length,
  };
}
...
function TickerReport({ report, study }) {
  const { ticker, executiveSummary, moveInContext, historicalPattern, currentEventAssessment, scores, timeHorizon, keyRisks } = report;
  const profile = study?.profile;
  const volatility = study?.volatility;
  return (
    <article className="analysis-report">
      <h2>{ticker} — Executive Summary</h2>
      {profile && profile.stability !== 'unknown' && (
        <p className="analysis-profile-chips">
          <span>{profile.marketCapTier}-cap</span>
          <span>{profile.stability}</span>
          {volatility && <span>normal daily swing ±{volatility.dailyVolPct}%</span>}
          {volatility && <span>annualized vol {volatility.annualizedVolPct}%</span>}
        </p>
      )}
      <p>{executiveSummary}</p>

      {moveInContext && <><h3>This Move in Context</h3><p>{moveInContext}</p></>}
...
export default function AnalysisTab({ live, events }) {
    ...
    try {
      const studies = await Promise.all(symbols.map(async (symbol) => {
        ...
        return {
          ...
          volatility: volatilityProfile(history),
  ...
  const ottoImage = loading ? ottoThinking : ottoWaiting;
  const ottoText = loading ? 'Otto is reviewing the evidence…' : 'Choose the evidence Otto should analyze.';
  // Once the report lands, Otto steps aside and the report takes the whole width —
  // these are dense tables and long-form text, and readability beats the mascot.
  const reportExpanded = reports.length > 0;
  ...
  return <div className="analysis-tab">
    ...
    <div className={`analysis-workspace card ${reportExpanded ? 'analysis-workspace--expanded' : ''}`}>{!reportExpanded && <aside className="analysis-otto" aria-live="polite"><img src={ottoImage} alt="Otto assistant" /><p>{ottoText}</p></aside>}<section className="analysis-panel">
      {step === 1 && <><div className="analysis-panel-heading"><span>Step 1</span><h2>Which holdings should Otto analyze?</h2><p>Choose one or more positions to compare their event exposure.</p></div><div className="analysis-stock-list">{positions.length ? positions.map((position) => <button type="button" className={`analysis-stock ${symbols.includes(position.symbol) ? 'selected' : ''}`} key={position.symbol} onClick={() => toggleSymbol(position.symbol)}><span className="analysis-check">{symbols.includes(position.symbol) ? '✓' : ''}</span><strong>{position.symbol}</strong><span>{usd(position.value)}</span><em className={position.dayChangePct >= 0 ? 'pos' : 'neg'}>{pct(position.dayChangePct)}</em></button>) : <p className="muted analysis-empty">Otto analyzes the stocks you hold. Add a position in the Portfolio tab to get started.</p>}</div><div className="analysis-actions"><span>{symbols.length} selected</span><button type="button" disabled={!symbols.length} onClick={() => setStep(2)}>Review event evidence →</button></div></>}
