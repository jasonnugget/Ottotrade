...
lines.push('-- Run AFTER supabase/schema.sql and supabase/migration-user-portfolio.sql.');
lines.push('-- Then run seed-bars-1.sql, seed-bars-2.sql, … in order (price bars are split out');
lines.push('-- into their own files because they are too large for the SQL Editor in one go).');
lines.push('');
// portfolio_lots is user data — never truncated here. Only the public reference tables are.
// demo_stock_bars is truncated by seed-bars-1.sql, not here.
...
// The price bars dwarf everything else, and the Supabase SQL Editor rejects a file much
// past ~1MB. So they go into their own numbered files, each small enough to paste, rather
// than one seed the editor won't accept.
const BAR_ROWS_PER_FILE = 2000;
const BARS_INSERT = 'insert into public.demo_stock_bars (ticker, interval, bar_ts, open, high, low, close, adjclose, volume) values';

fs.mkdirSync(path.dirname(outputFile), { recursive: true });

// Remove bar files from a previous (possibly larger) run so no stale chunk gets applied.
for (const name of fs.readdirSync(path.dirname(outputFile))) {
  if (/^seed-bars-\d+\.sql$/.test(name)) fs.unlinkSync(path.join(path.dirname(outputFile), name));
}

fs.writeFileSync(outputFile, lines.join('\n'));
console.log(`Wrote ${outputFile} (${(fs.statSync(outputFile).size / 1024).toFixed(0)} KB)`);

const chunkCount = Math.ceil(barRows.length / BAR_ROWS_PER_FILE);
for (let index = 0; index < chunkCount; index += 1) {
  const chunk = barRows.slice(index * BAR_ROWS_PER_FILE, (index + 1) * BAR_ROWS_PER_FILE);
  const file = path.join(path.dirname(outputFile), `seed-bars-${index + 1}.sql`);
  const body = [
    `-- Price bars ${index + 1} of ${chunkCount}. Run seed.sql first, then these in order.`,
    // Only the first chunk clears the table; the rest append to it.
    index === 0 ? 'truncate table public.demo_stock_bars restart identity cascade;\n' : '',
    BARS_INSERT,
    chunk.join(',\n') + ';',
    '',
  ].join('\n');
  fs.writeFileSync(file, body);
  console.log(`Wrote ${file} (${(fs.statSync(file).size / 1024).toFixed(0)} KB, ${chunk.length} rows)`);
}
